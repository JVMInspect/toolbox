...
interface Kernel32Ext : Kernel32 {
    ...
    fun GetProcAddress(handle: HMODULE, name: String): LPVOID?

    @Suppress("FunctionName")
    fun ReadProcessMemory(handle: HANDLE, address: Long, buffer: Long, size: Int, read: IntByReference): Boolean
    ...
    @Suppress("FunctionName")
    fun WriteProcessMemory(handle: HANDLE, address: Long, buffer: Long, size: Int, read: IntByReference): Boolean
...
fun iovec(base: Long, len: Long): iovec = iovec().apply {
    iov_base = base
    iov_len = len
}
...
interface LibCExt : LibC {
    @Suppress("FunctionName")
    fun process_vm_readv(pid: Int, local: Long, liovcnt: Int, remote: Long, riovcnt: Int, flags: Long): Int

    @Suppress("FunctionName")
    fun process_vm_writev(pid: Int, local: Long, liovcnt: Int, remote: Long, riovcnt: Int, flags: Long): Int
