...
abstract class BaseArrayFieldDelegate<E : Any, A : NArray<E>>(
    val struct: Struct,
...
open class NullableArrayFieldDelegate<E : Any, A : NArray<E>>(
    struct: Struct,
    ...
    open operator fun setValue(struct: Struct, property: KProperty<*>, value: A?) {
        unsafe.putAddress(address(false), value?.address?.base ?: 0)
    }
}

class ArrayFieldDelegate<E : Any, A : NArray<E>>(
    struct: Struct,
    arrayType: KClass<in A>,
    elementType: KClass<E>,
    isElementPointer: Boolean,
    ...
    override operator fun getValue(struct: Struct, property: KProperty<*>): A =
        super.getValue(struct, property) ?: throw NullPointerException("${struct.vmTypeName}#$location")
}

inline fun <reified E : Any, reified A : NArray<E>?> Struct.maybeNullArray(
    fieldName: String,
...
inline fun <reified E : Any, reified A : NArray<E>> Struct.nonNullArray(
    fieldName: String,
    isElementPointer: Boolean = structs.isStruct(E::class)
) = ArrayFieldDelegate(
    struct = this,
