...
class Fields(scope: Scope) : Scope by scope {
    ...
    fun offset(struct: Struct, name: String) =
        offsets[struct::class to name] ?: error("${struct.vmType.name}#$name has not been mapped to an offset!")

    fun address(struct: Struct, name: String) =
        addresses[struct::class to name] ?: error("${struct.vmType.name}#$name has not been mapped to an address!")
        ...
        when (location) {
            is FieldLocation.Name -> {
                val field = this(struct, location.value) ?:
                    error("Could not find ${struct.vmType.name}#${location.value}")
    ...
    operator fun invoke(struct: Struct, name: String) = cache.computeIfAbsent(struct::class to name) {
        struct.vmType.field(name) ?: supers(struct::class).firstNotNullOfOrNull {
            structs(Address.PLACEHOLDER, it)?.vmType?.field(name)
        }
    }
}